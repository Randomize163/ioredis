...
Redis.prototype._readyCheck = function (callback) {
    ...
    this.info(function (err, res) {
        ...
        if (!info.loading || info.loading === '0') {
            ...
            const retryTime = _this.options.maxLoadingRetryTime && _this.options.maxLoadingRetryTime < loadingEtaMs
...
Redis.prototype.sendCommand = function (command, stream) {
    ...
    let writable = this.status === 'ready' ||
        (!stream && this.status === 'connect' && commands.exists(command.name) && commands.hasFlag(command.name, 'loading'));
    if (!this.stream) {
        writable = false;
    }
