...
import * as NetUtils from 'net-keepalive';

const debug = Debug('connection');

function configureSocketOptions(self) {
    const socket = self.stream;
    const { keepAliveInterval, keepAliveInitialDelay, keepAliveProbesCount, tcpUserTimeout } = self.options;

    const shouldEnableKeepAlive = keepAliveInterval || keepAliveInitialDelay || keepAliveProbesCount || tcpUserTimeout;
    ...
    if (typeof keepAliveInitialDelay === 'number') {
        socket.setKeepAlive(true, keepAliveInitialDelay);
    }

    if (typeof keepAliveInterval === 'number') {
        NetUtils.setKeepAliveInterval(self.stream, keepAliveInterval);
    }

    if (typeof keepAliveProbesCount === 'number') {
        NetUtils.setKeepAliveProbes(self.stream, keepAliveProbesCount);
    }

    if (typeof tcpUserTimeout === 'number') {
        NetUtils.setUserTimeout(self.stream, tcpUserTimeout);
    }
}
